...
export default function GraphView({ posts }: GraphViewProps) {
    ...
    const { nodes, edges } = useMemo(() => {
        ...
        const edges: Edge[] = posts
            ...
            .map(post => ({
                ...
                source: post.parent_post_id!.toString(),
    ...
    // 2. Sentiment Data (Donut)
    const sentimentData = useMemo(() => {
        const proCount = posts.filter(p => p.side === 'PRO').length;
        const conCount = posts.filter(p => p.side === 'CON').length;
        return [
            { name: 'Support', value: proCount, color: '#3B82F6' },
            { name: 'Oppose', value: conCount, color: '#F43F5E' },
        ];
    }, [posts]);

    // 3. Interaction Data (Bar)
